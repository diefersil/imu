 ...
 * GFExcel_VendorHTMLPurifier_Lexer_DirectLex,and is the default choice for PHP 5.
 *
...
class GFExcel_VendorHTMLPurifier_Lexer_DOMLex extends GFExcel_VendorHTMLPurifier_Lexer
{

    /**
     * @type GFExcel_VendorHTMLPurifier_TokenFactory
     */
    private $factory;
    ...
    public function __construct()
    {
        // setup the factory
        parent::__construct();
        $this->factory = new GFExcel_VendorHTMLPurifier_TokenFactory();
    }
     ...
     * @param GFExcel_VendorHTMLPurifier_Token[] $tokens   Array-list of already tokenized tokens.
     */
    protected function tokenizeDOM($node, &$tokens, $config)
    {
        $level = 0;
        $nodes = array($level => new GFExcel_VendorHTMLPurifier_Queue(array($node)));
        $closingNodes = array();
        do {
            while (!$nodes[$level]->isEmpty()) {
                ...
                if ($node->childNodes && $node->childNodes->length) {
                    $level++;
                    $nodes[$level] = new GFExcel_VendorHTMLPurifier_Queue();
    ...
    protected function createStartNode($node, &$tokens, $collect, $config)
    {
        ...
        if ($node->nodeType === XML_TEXT_NODE) {
            ...
            if ($last instanceof GFExcel_VendorHTMLPurifier_Token_Start && ($last->name == 'script' || $last->name == 'style')) {
                $new_data = trim($data);
