 ...
 * GFExcel_VendorHTMLPurifier_Lexer_DOMLex).  There are several implementations to choose
 * from.
 *
...
class GFExcel_VendorHTMLPurifier_Lexer
{
    ...
    /**
     * @type GFExcel_VendorHTMLPurifier_EntityParser
     */
     ...
     *
     * By default GFExcel_VendorHTMLPurifier_Lexer_DOMLex will be returned. There are
    ...
    public static function create($config)
    {
        if (!($config instanceof GFExcel_VendorHTMLPurifier_Config)) {
            ...
            trigger_error(
                ...
                GFExcel_VendorHTMLPurifier_Lexer::create() is deprecated, please instead
                use %Core.LexerImpl",
                E_USER_WARNING
            );
        } else {
        ...
        if (is_object($lexer)) {
            ...
            switch ($lexer) {
                case 'DOMLex':
                    $inst = new GFExcel_VendorHTMLPurifier_Lexer_DOMLex();
                    break;
                case 'DirectLex':
                    $inst = new GFExcel_VendorHTMLPurifier_Lexer_DirectLex();
                    break;
                case 'PH5P':
                    $inst = new GFExcel_VendorHTMLPurifier_Lexer_PH5P();
                    break;
                default:
        ...
        if (!$inst) {
            throw new GFExcel_VendorHTMLPurifier_Exception('No lexer was instantiated');
        }
    ...
    {
        $this->_entity_parser = new GFExcel_VendorHTMLPurifier_EntityParser();
    }
    ...
    protected static function escapeCDATA($string)
    {
        return preg_replace_callback(
            '/<!\[CDATA\[(.+?)\]\]>/s',
            array('GFExcel_VendorHTMLPurifier_Lexer', 'CDATACallback'),
            $string
        );
    }
    ...
    public function normalize($html, $config, $context)
    {
        ...
        $html = GFExcel_VendorHTMLPurifier_Encoder::cleanUTF8($html);
