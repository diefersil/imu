 ...
 * pales in comparison to GFExcel_VendorHTMLPurifier_Lexer_DOMLex.
 *
    ...
    public function tokenizeHTML($html, $config, $context)
    {
        ...
        while (++$loops) {
            ...
            if (!$inside_tag && $position_next_lt !== false) {
                ...
                $token = new
                GFExcel_VendorHTMLPurifier_Token_Text(
                    $this->parseText(
                        substr(
                            $html,
                            $cursor,
                ...
                if (substr($segment, 0, 3) === '!--') {
                    ...
                    $segment = substr($html, $cursor, $strlen_segment);
                    $token = new
                    GFExcel_VendorHTMLPurifier_Token_Comment(
                        substr(
                            $segment,
                            3,
                            $strlen_segment - 3
                ...
                if ($position_first_space >= $strlen_segment) {
                    if ($is_self_closing) {
                        $token = new GFExcel_VendorHTMLPurifier_Token_Empty($segment);
                    } else {
                        $token = new GFExcel_VendorHTMLPurifier_Token_Start($segment);
                    }
                    if ($maintain_line_numbers) {
                        $token->rawPosition($current_line, $current_col);
                        $current_line += $this->substrCount($html, $nl, $cursor, $position_next_gt - $cursor);
                ...
                if ($is_self_closing) {
                    $token = new GFExcel_VendorHTMLPurifier_Token_Empty($type, $attr);
                } else {
                    $token = new GFExcel_VendorHTMLPurifier_Token_Start($type, $attr);
                }
                if ($maintain_line_numbers) {
                    $token->rawPosition($current_line, $current_col);
                    $current_line += $this->substrCount($html, $nl, $cursor, $position_next_gt - $cursor);
