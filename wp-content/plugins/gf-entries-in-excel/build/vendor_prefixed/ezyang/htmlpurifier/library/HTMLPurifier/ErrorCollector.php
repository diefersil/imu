...
class GFExcel_VendorHTMLPurifier_ErrorCollector
{
    ...
    /**
     * @type GFExcel_VendorHTMLPurifier_Language
     */
    protected $locale;
    ...
    public function send($severity, $msg)
    {
        ...
        $new_struct = new GFExcel_VendorHTMLPurifier_ErrorStruct();
        $new_struct->type = GFExcel_VendorHTMLPurifier_ErrorStruct::TOKEN;
        if ($token) {
        ...
        if (!empty($attr)) {
            $struct = $struct->getChild(GFExcel_VendorHTMLPurifier_ErrorStruct::ATTR, $attr);
            if (!$struct->value) {
                $struct->value = array($attr, 'PUT VALUE HERE');
            }
        }
        if (!empty($cssprop)) {
            $struct = $struct->getChild(GFExcel_VendorHTMLPurifier_ErrorStruct::CSSPROP, $cssprop);
            if (!$struct->value) {
    ...
    public function getHTMLFormatted($config, $errors = null)
    {
        ...
        $this->generator = new GFExcel_VendorHTMLPurifier_Generator($config, $this->context);
        if ($errors === null) {
            $errors = $this->errors;
        }
