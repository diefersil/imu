...
if ( ! class_exists( 'Jet_Engine' ) ) {
	...
	class Jet_Engine {
		...
		public function activation() {
			...
			$tables = Jet_Engine_DB::tables();

			if ( ! Jet_Engine_DB::table_exists( $tables['post_types']['name'] ) ) {
				$this->on_clean_install();
			}

			Jet_Engine_DB::create_all_tables();
		}

		private function on_clean_install() {
			$this->init_misc_settings();
			$this->misc_settings->on_clean_install();
		}
