...
if ( ! class_exists( 'Jet_Engine_Listings_Data' ) ) {
	...
	class Jet_Engine_Listings_Data {
		...
		public function get_object_type( $object ) {
			...
			if ( isset( $object->_ID ) && isset( $object->cct_slug ) && is_string( $object->cct_slug ) ) {
				return 'jet_engine_cct_' . $object->cct_slug;
			}

			return get_class( $object );
		}

		public function get_current_object_unique_id( $object = null ) {
			if ( ! is_object( $object ) ) {
				return false;
			}
			...
			if ( empty( $this->unique_ids[ $key ] ) ) {
				$this->unique_ids[ $key ] = sprintf( '%.0f', hrtime( true ) );
			}

			return $this->unique_ids[ $key ];
		}
