...
class Manager extends \Jet_Engine_Base_WP_Intance {
	...
	public function get_query_count_html( $query_id = false, $count_type = false ) {
		$result = $this->get_query_count( $query_id, $count_type );

		return sprintf(
			'<span class="jet-engine-query-count query-%2$s count-type-%3$s" data-query="%2$s">%1$s</span>',
			$result,
			esc_attr( $query_id ),
			esc_attr( $count_type )
		);
	...
	public function get_query_count( $query_id = false, $count_type = false ) {

		if ( ! $count_type ) {
			$count_type = 'total';
		}

		$allowed_count_types = array(
			'total',
			'visible',
			'start-item',
			'end-item',
		...
		if ( ! in_array( $count_type, $allowed_count_types, true ) ) {
			$count_type = 'total';
		}
