...
class Jet_Engine_Base_DB {
		...
		if ( ! empty( $args ) ) {
			...
			foreach ( $args as $key => $arg ) {
				...
				if ( isset( $arg['relation'] ) ) {
					$sub_rel = $this->sanitize_sql_relation( $arg['relation'] );
					unset( $arg['relation'] );
	...
	/**
	 * Normalize SQL comparison operator against JetEngine allowlist.
	 *
	 * @param  string $operator Requested operator.
	 * @return string
	 */
	public function sanitize_sql_operator( $operator = '=' ) {
	...
	/**
	 * Normalize nested relation glue for SQL where groups.
	 *
	 * @param  string $relation Requested relation.
	 * @return string
	 */
	public function sanitize_sql_relation( $relation = 'AND' ) {
	...
	/**
	 * Validate field name used in SQL where clauses.
	 *
	 * @param  mixed $field Field name.
	 * @return string|false
	 */
	public function sanitize_sql_field( $field = false ) {
		...
		if ( ! $field || ! preg_match( '/^[a-zA-Z0-9_]+(\.[a-zA-Z0-9_]+)*$/', $field ) ) {
			return false;
		}
	...
	/**
	 * Quote a safe SQL identifier or dotted identifier.
	 *
	 * @param  string $field Sanitized field name.
	 * @return string
	 */
	public function quote_sql_field( $field ) {
		$segments = explode( '.', $field );
		return '`' . implode( '`.`', $segments ) . '`';
	}

	/**
	 * Allow only safe DECIMAL casts for direct SQL interpolation.
	 *
	 * @param  mixed $type Requested type.
	 * @return string|false
	 */
	public function sanitize_sql_cast_type( $type = false ) {
		...
		if ( 'DECIMAL' === $type || preg_match( '/^DECIMAL\(\d+,\d+\)$/', $type ) ) {
			return $type;
		}
	...
	public function prepare_where_clause( $field = false, $operator = '=', $value = '', $type = false ) {
		...
		$operator       = $this->sanitize_sql_operator( $operator );
		$sql_cast_type  = $this->sanitize_sql_cast_type( $type );

		if ( ! $field ) {
			return '';
		}

		$field_sql       = $this->quote_sql_field( $field );
		$format          = '%1$s %3$s %2$s';
		$array_operators = array( 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN' );

		if ( $sql_cast_type ) {
			$format = 'CAST( %1$s AS ' . $sql_cast_type . ' ) %3$s %2$s';
		}
		...
		$result = sprintf( $format, $field_sql, $value, $operator );
