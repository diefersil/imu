	...
	public function posts_where( $sql, $query ) {
		...
		if ( $geo_query ) {
			...
			if ( $this->must_apply_bounds( $geo_query ) ) {
				...
				$new_sql = " ( {$lat_field} BETWEEN {$bounds['south']} AND {$bounds['north']}";

				//if map includes 180deg meridian and western bound is greater than eastern
				if ( $bounds['west'] >= $bounds['east'] ) {
					$new_sql .= " AND ( {$lng_field} >= {$bounds['west']} OR {$lng_field} <= {$bounds['east']} ) )";
				} else {
					$new_sql .= " AND {$lng_field} BETWEEN {$bounds['west']} AND {$bounds['east']} )";
				}
			} else {
