...
class Jet_Accordion_Widget extends Jet_Tabs_Base {
	...
	protected function render() {
		...
		$content_scrolling_offset_desktop = $this->get_settings_for_display( 'content_scrolling_offset' );
		$content_scrolling_offset_tablet  = $this->get_settings_for_display( 'content_scrolling_offset_tablet' );
		$content_scrolling_offset_mobile  = $this->get_settings_for_display( 'content_scrolling_offset_mobile' );
		...
		$settings = array(
			...
			'switchScrollingTablet' => filter_var($this->get_settings('content_scrolling_tablet'), FILTER_VALIDATE_BOOLEAN),
			'switchScrollingMobile' => filter_var($this->get_settings('content_scrolling_mobile'), FILTER_VALIDATE_BOOLEAN),


			'switchScrollingOffset'       => ! empty($content_scrolling_offset_desktop) ? $content_scrolling_offset_desktop : array(
				'size' => 0,
				'unit' => 'px',
			),
			'switchScrollingOffsetTablet' => ! empty($content_scrolling_offset_tablet) ? $content_scrolling_offset_tablet : array(
				'size' => 0,
				'unit' => 'px',
			),
			'switchScrollingOffsetMobile' => ! empty($content_scrolling_offset_mobile) ? $content_scrolling_offset_mobile : array(
				'size' => 0,
				'unit' => 'px',
			),

			'switchScrollingDelay'       => ! empty($this->get_settings_for_display('content_scrolling_delay')) ? (int)$this->get_settings_for_display('content_scrolling_delay') : 500,
			'switchScrollingDelayTablet' => ! empty($this->get_settings_for_display('content_scrolling_delay_tablet')) ? (int)$this->get_settings_for_display('content_scrolling_delay_tablet') : 500,
			'switchScrollingDelayMobile' => ! empty($this->get_settings_for_display('content_scrolling_delay_mobile')) ? (int)$this->get_settings_for_display('content_scrolling_delay_mobile') : 500,
		);
							...
							case 'template':
								...
								if ( ! empty( $item['item_template_id'] ) ) {
									...
									$template_content = apply_filters(
										...
										$template_content,
										$item,
										'jet-toggle-content-' . $id_int . $toggle_count,
										$this
									);
									...
									if ( ! empty( $template_content ) ) {
										...
										if ( jet_tabs_integration()->is_edit_mode() ) {
											...
											$content_html .= sprintf(
												'<div class="jet-toggle__edit-cover" data-template-edit-link="%s"><i class="fas fa-pencil-alt"></i><span>%s</span></div>',
												esc_url( $link ),
