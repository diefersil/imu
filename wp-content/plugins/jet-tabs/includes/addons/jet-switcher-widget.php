...
class Jet_Switcher_Widget extends Jet_Tabs_Base {
	...
	public function get_template_content_by_id( $template_id ) {
		...
		if ( ! empty( $template_id ) ) {
			...
			if ( ! $template_post ) {
				return $this->no_templates_message();
			}

			if ( 'elementor_library' !== $template_post->post_type ) {
				return $this->no_template_content_message();
			}
			...
			if ( ! empty( $template_content ) ) {
				...
				if ( jet_tabs_integration()->is_edit_mode() ) {
					...
					$content_html .= sprintf(
						'<div class="jet-switcher__edit-cover" data-template-edit-link="%s"><i class="fas fa-pencil-alt"></i><span>%s</span></div>',
						esc_url( $link ),
