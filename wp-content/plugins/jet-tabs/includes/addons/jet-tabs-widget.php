...
class Jet_Tabs_Widget extends Jet_Tabs_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'flip_text_direction',
			array(
				'label'              => esc_html__( 'Flip Text Direction', 'jet-tabs' ),
				'type'               => Controls_Manager::SWITCHER,
		...
		$this->__add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .jet-tabs.jet-tabs-flip-direction.jet-tabs-position-left > .jet-tabs__control-wrapper'  => 'min-width: 0; width: auto; flex: 0 0 auto;',
					'{{WRAPPER}} .jet-tabs.jet-tabs-flip-direction.jet-tabs-position-right > .jet-tabs__control-wrapper' => 'min-width: 0; width: auto; flex: 0 0 auto;',

					'{{WRAPPER}} .jet-tabs.jet-tabs-flip-direction.jet-tabs-position-left > .jet-tabs__content-wrapper'  => 'min-width: 0; flex: 1 1 auto;',
					'{{WRAPPER}} .jet-tabs.jet-tabs-flip-direction.jet-tabs-position-right > .jet-tabs__content-wrapper' => 'min-width: 0; flex: 1 1 auto;',

					'{{WRAPPER}} .jet-tabs.jet-tabs-flip-direction.jet-tabs-position-left > .jet-tabs__control-wrapper > .jet-tabs__control'  => 'min-height: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .jet-tabs.jet-tabs-flip-direction.jet-tabs-position-right > .jet-tabs__control-wrapper > .jet-tabs__control' => 'min-height: {{SIZE}}{{UNIT}};',
				),
			),
			25
		);
			...
			case 'template':
				if ( ! empty( $item['item_template_id'] ) ) {
					...
					$template_content = apply_filters(
						'jet-tabs/widgets/template_content',
						$template_content,
						$item,
						'jet-tabs-content-' . $id_int . $tab_count,
						$this
					);
					...
					if ( ! empty( $template_content ) ) {
						...
						if ( jet_tabs_integration()->is_edit_mode() ) {
							...
							$content_html .= sprintf(
								'<div class="jet-tabs__edit-cover" data-template-edit-link="%s"><i class="fas fa-pencil-alt"></i><span>%s</span></div>',
								esc_url( $link ),
								esc_html__( 'Edit Template', 'jet-tabs' )
							);
						}
					} else {
						$content_html = $this->no_template_content_message();
	...
	protected function render() {
		...
		$flip_text_direction  = filter_var( $this->get_settings( 'flip_text_direction' ), FILTER_VALIDATE_BOOLEAN );
		...
		$attributes = array(
			'class'         => array(
				...
				( $flip_text_direction ) ? 'jet-tabs-flip-direction' : '',
			),
		...
		if ( $tabs_scrolling_navigation && $tab_scroll_type === 'slider' && ( $tabs_position === 'top' || $tabs_position === 'bottom' ) ) {
			...
			$attributes['data-swiper-settings'] = wp_json_encode( $swiper_settings );
		}

		$this->add_render_attribute( 'instance', $attributes );

		$control_width = $this->get_settings_for_display( 'tabs_control_wrapper_width' );

		if (
			! $flip_text_direction &&
			! empty( $control_width['size'] ) &&
			! empty( $control_width['unit'] ) &&
			in_array( $tabs_position, [ 'left', 'right' ], true )
		) {

			$size = $control_width['size'];
			$unit = $control_width['unit'];

			// Apply width to controls wrapper
			$this->add_render_attribute( 'control_wrapper', 'style', sprintf( 'min-width:%1$s%2$s;', $size, $unit ) );

			$this->add_render_attribute(
				'control_wrapper',
				'style',
				sprintf( 'min-width:%1$s%2$s;width:%1$s%2$s;flex:0 0 %1$s%2$s;', $size, $unit )
			);
		}
		...
		?>>
			<?php
				$this->add_render_attribute( 'control_wrapper', 'class', 'jet-tabs__control-wrapper' );
				$this->add_render_attribute( 'control_wrapper', 'role', 'tablist' );
				$this->add_render_attribute( 'content_wrapper', 'class', 'jet-tabs__content-wrapper' );

				if ( $tabs_scrolling_navigation && $tab_scroll_type === 'plain' ) {
					$this->add_render_attribute( 'control_wrapper', 'class', 'jet-tabs-scrolling-navigation' );
				}

				if ( $tabs_scrolling_navigation && $tab_scroll_type === 'slider' && ( $tabs_position === 'top' || $tabs_position === 'bottom' ) ) {
					$this->add_render_attribute( 'control_wrapper', 'class', 'jet-tabs-swiper' );
				}
			?>
			<div <?php echo $this->get_render_attribute_string( 'control_wrapper' ); ?>>
                    ...
                    <?php
                        ...
                        foreach ( $tabs as $index => $item ) {
                            ...
                            if ( ! empty( $item['item_image']['url'] ) ) {
                                $title_image_html = sprintf( '<img class="jet-tabs__label-image" src="%1$s" alt="">', esc_url( $item['item_image']['url'] ) );
                            }

                            $title_label_html = '';

                            if ( ! empty( $item['item_label'] ) ) {
                                $title_label_html = sprintf( '<' . $tabs_item_label_tag . ' class="jet-tabs__label-text">%1$s</' . $tabs_item_label_tag . '>', wp_kses_post( $item['item_label'] ) );
                            }
	...
	public function get_template_edit_link( $template_id ) {
		...
		return '<a target="_blank" class="elementor-edit-template elementor-clickable" href="' . esc_url( $link ) .'"><i class="fas fa-pencil"></i> ' . esc_html__( 'Edit Template', 'jet-tabs' ) . '</a>';
	}
