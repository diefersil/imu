...
## 2.3.0
* Added: Flip Text Direction for left and right tab layouts.
* Added: Responsive “Scrolling to the Content” settings for Classic Accordion.

## 2.2.14
* Fixed: Classic Accordion Ajax “Current Query” template loading.
* Fixed: Tabs control wrapper width when used via shortcode.
* Fixed: Switcher anchor links inside listings (correct smooth scroll + no layout breaks).
* Fixed: Image Accordion “Title Default” no longer overlaps clickable content.
* Updated: Added content filtering support and dynamic styles handling for Accordion/Tabs content.

## 2.2.13
* Added: Videos now auto-pause when switching between tabs.
* Fixed: Auto Switch no longer reveals content of tabs hidden by the Hide Empty Items add-on.
* Fixed: Arrow key navigation is now blocked inside inputs/textareas to prevent unintended tab/listing navigation (including Review Listing).

## 2.2.12.1
* Fixed: Potential XSS vulnerability in Tabs/Accordion widgets.
