...
function ac() {?>
	<script type="module" defer>
		//var countries = <?php //include(get_template_directory() . '/functions-ac-tags.php'); ?>
		import { locais } from '<?php include(get_template_directory() . '/functions-ac.js'); ?>
		autocomplete(document.getElementById("search-ac"), locais);
		function autocomplete(inp, arr) {
			...
			inp.addEventListener("input", function(e) {
				...
				for (i = 0; i < arr.length; i++) {
					...
					const str = arr[i].nome.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase();
					const str2 = val.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase();
					const re = new RegExp(str2, "i");
					if (re.exec(str) && arr[i].nome !== 'Title' && arr[i].tipo == 'marca') {
    					//if (arr[i].nome.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
    					//if (arr[i].nome.replace(/[\u0300-\u036f]/g, "").toLowerCase().includes(val.replace(/[\u0300-\u036f]/g, "").toLowerCase())) {/*ADD*/
